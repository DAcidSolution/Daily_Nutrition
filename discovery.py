...
def open_discovery():
    ...
    driver = custom_driver.get_custom_driver(
        name=config.get_browser(),
        options=od_dict.get('options'),
        desired_capabilities=od_dict.get('desired_capabilities'))
        ...
        for i in range(config.get_retry()):
            try:
                print(f'access num:{i}',
                      tu.get_name(opener=opener, headers=headers))
                break
            except Exception as e:
                print(e)
        ...
        for i in range(config.get_retry()):
            try:
                ...
                for i, p in enumerate(temp):
                    print(f'p{i}:',
                          p.get('urls').get(config.get_image_quality()))
                break
            except Exception as e:
                print(e)
